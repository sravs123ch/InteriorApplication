        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              {fromDashboard ? (
      <StyledTableCell style={{ width: "15%" }}>Customer Status</StyledTableCell>
    ) : (
      <StyledTableCell style={{ width: "15%" }}>Gender</StyledTableCell>
    )}
            ...
            {isLoading ? (
              ...
              filteredCustomers.map((person, index) => (
                <StyledTableRow key={index}>
                  <StyledTableCell>{person.CustomerNumber?person.CustomerNumber:"N/A"}</StyledTableCell>
                  <StyledTableCell>
...
<StyledTableCell>
  {fromDashboard ? (
    ...
    <span
      className={`flex items-center justify-center min-w-[40px] px-3 h-9 rounded-3xl text-white text-sm font-semibold ${
        statusColors[person.CustomerStatus] || "bg-gray-400"
      }`}
    >
      {person.CustomerStatus || "N/A"}
    </span>
  ) : (
    // Gender Pill
    <span
    ...
    >
      {person.Gender === null ? "N/A" : person.Gender === "M" ? "Male" : "Female"}
    </span>
  )}
</StyledTableCell>
...
<button
  ...
  className={`button view-button w-32 whitespace-nowrap ${
    !ordersMap[person.CustomerID] ? "opacity-50 cursor-not-allowed" : ""
  }`}
  disabled={!ordersMap[person.CustomerID]} // Disable if OrderCount is 0
>
