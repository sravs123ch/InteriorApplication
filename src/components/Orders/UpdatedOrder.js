...
export const UpdatedStatusOrderProvider = ({ children }) => {
  ...
  useEffect(() => {}, [updatedStatusOrderDetails]);

  return (
    <OrderUpdatedContext.Provider
      value={{ updatedStatusOrderDetails, setUpdatedStatusOrderDetails }}
    >
      {children}
    </OrderUpdatedContext.Provider>
  );
...
export const useUpdatedStatusOrderContext = () =>
  useContext(OrderUpdatedContext);
